<script lang="ts">
    import AppComp from "../components/AppComp.svelte";
...
{#if type === "all"}
{#each data as app}
    <AppComp ipa={app.ipa} imageData={app.image} name={app.name} />
{/each}
{:else if type ==="jailbreaks"}
{#each data as app}
    {#if app.category === "jailbreak"}
        <AppComp ipa={app.ipa} imageData={app.image} name={app.name} />
    {/if}
{/each}
{:else if type === "tweaks"}
{#each data as app}
    {#if app.category === "tweaks"}
        <AppComp ipa={app.ipa} imageData={app.image} name={app.name} />
    {/if}
{/each}
{/if}